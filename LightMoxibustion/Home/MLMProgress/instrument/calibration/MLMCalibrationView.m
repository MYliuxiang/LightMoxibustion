        ...
        if (i%_smallScaleNum == 0) {
            ...
            UIBezierPath *majorPath = [UIBezierPath bezierPathWithArcCenter:CGPointMake(self.width/2, self.height/2)
                                                                 ...
                                                                 startAngle:DEGREES_TO_RADIANS(startAngle)
                                                                  ...
                                                                  clockwise:YES];
            CGFloat start = DEGREES_TO_RADIANS(startAngle);
            UIBezierPath *aPath = [UIBezierPath bezierPath];
            [aPath moveToPoint:(CGPoint){ self.width/2 + cosf(start) * (circleRadius + _majorScaleLength / 2),  self.height/2 + sinf(start) * (circleRadius + _majorScaleLength / 2)}];
            [aPath addLineToPoint:(CGPoint){  self.width/2 + cosf(start) * (circleRadius - _majorScaleLength / 2), self.height/2 + sinf(start) * (circleRadius - _majorScaleLength / 2)}];
            
            
            CAShapeLayer *majorLayer = [CAShapeLayer layer];
            majorLayer.strokeColor = _majorScaleColor.CGColor;
            majorLayer.path = [aPath CGPath];
            majorLayer.lineWidth = _majorScaleWidth;
            majorLayer.lineCap = kCALineCapRound;
            majorLayer.lineJoin = kCALineCapRound;
            ...
            UIBezierPath *smallPath = [UIBezierPath bezierPathWithArcCenter:CGPointMake(self.width/2, self.height/2) radius:circleRadius + (_majorScaleLength - _smallScaleLength) / 2
                                                                 startAngle:DEGREES_TO_RADIANS(startAngle)
                                                                   endAngle:DEGREES_TO_RADIANS(endAngle)
                                                                  clockwise:YES];
            ...
            UIBezierPath *aPath = [UIBezierPath bezierPath];
            [aPath moveToPoint:(CGPoint){ self.width/2 + cosf(start) * (circleRadius + _majorScaleLength / 2),  self.height/2 + sinf(start) * (circleRadius + _majorScaleLength / 2)}];
            [aPath addLineToPoint:(CGPoint){  self.width/2 + cosf(start) * (circleRadius + _majorScaleLength / 2 - _smallScaleLength / 2), self.height/2 + sinf(start) * (circleRadius + _majorScaleLength / 2 - _smallScaleLength / 2)}];

            CAShapeLayer *smallLayer = [CAShapeLayer layer];
            smallLayer.strokeColor = _smallScaleColor.CGColor;
            smallLayer.path = [aPath CGPath];
            smallLayer.lineWidth = _smallScaleWidth;
            smallLayer.lineCap = kCALineCapRound;
            smallLayer.lineJoin = kCALineCapRound;
...
//    }
    ...
    for (NSInteger i = 0; i < textArray.count; i ++) {
        ...
        //对应的center
        CGFloat textRadius = circleRadius + _majorScaleLength + _scaleSpace;
...
- (CGFloat)outCircrDiameter{
    return (circleRadius + _majorScaleLength) * 2 ;
}
